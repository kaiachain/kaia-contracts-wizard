...
function addBase(c: ContractBuilder, name: string, symbol: string) {
  c.addParent(
    {
      name: 'KIP17',
      path: '@kaiachain/contracts/KIP/token/KIP17/KIP17.sol',
    },
    [name, symbol],
  );
...
function addEnumerable(c: ContractBuilder) {
  c.addParent({
    ...
    path: '@kaiachain/contracts/KIP/token/KIP17/extensions/KIP17Enumerable.sol',
  });
...
function addURIStorage(c: ContractBuilder) {
  c.addParent({
    ...
    path: '@kaiachain/contracts/KIP/token/KIP17/extensions/KIP17URIStorage.sol',
  });
...
function addBurnable(c: ContractBuilder) {
  c.addParent({
    name: 'KIP17Burnable',
    path: '@kaiachain/contracts/KIP/token/KIP17/extensions/KIP17Burnable.sol',
  });
...
function addMintable(c: ContractBuilder, access: Access, incremental = false, uriStorage = false) {
  ...
  if (incremental) {
    c.addUsing({
      name: 'Counters',
      path: '@kaiachain/contracts/utils/Counters.sol',
    }, 'Counters.Counter');
...
function addVotes(c: ContractBuilder, name: string) {
  c.addParent(
    {
      name: 'EIP712',
      path: '@kaiachain/contracts/utils/cryptography/draft-EIP712.sol',
    },
    [name, "1"]
  );
  c.addParent(
    {
      name: 'KIP17Votes',
      path: '@kaiachain/contracts/KIP/token/KIP17/extensions/draft-KIP17Votes.sol',
    });
